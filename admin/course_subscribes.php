...
// Records List
if ($action == 'list'){
  $query = "SELECT * FROM course_subscribes";
  if ($find != ''){
    $query = $query . " WHERE (user_id LIKE '%$find%' OR course_id LIKE '%$find%')";
  }
  if ($orderby != ''){
    $query = $query . " ORDER BY $orderby";
  }
  print "<h1>Список подписчиков на курсы</h1>\n";
  ...
  $db->dbh = $dbh;
  $db->sql = $query;
  $db->template = 'course_subscribes.inc';
  $db->recperpage = 20;
  $db->execute();
  ...
  print "<input type=\"submit\" value=\"Добавить нового подписчика на курс\"></form>\n";
}
...
if ($action == 'confirm'){
  ...
  print "<table border=\"1\">\n";
  print "<tr><td colspan=\"2\">Вы действительно хотите удалить запись?</td></tr>\n";
...
function print_form($row) {
  ...
  print_form_list('users', 'n', '2');
  print "</td></tr>\n";
  print "<tr><td>course_id: </td><td>";
  print_form_list('courses', 'n', 'n');
  print "</td></tr>\n";
